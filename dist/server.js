...
let serverLifecycleState = "starting";
let shutdownPromise = null;
function resolveProviderIdOrHttpError(provider) {
    try {
...
function parsePositiveIntEnv(value, fallback, min) {
...
const metricsHistogramBucketBoundsMs = [10, 25, 50, 100, 250, 500, 1_000, 2_500, 5_000, 10_000];
const metricsHistogramBuckets = metricsHistogramBucketBoundsMs.map((upperBoundMs) => ({
    upperBoundMs,
    count: 0
}));
const metricsStatusCounts = new Map();
const metricsMethodCounts = new Map();
...
function recordHttpRequestMetric(method, statusCode, durationMs) {
...
function recordReadinessProbeMetric(ok, durationMs) {
...
function formatPrometheusMetricLine(metricName, value, labels) {
...
function renderPrometheusMetrics() {
    ...
    for (const state of lifecycleStates) {
        lines.push(formatPrometheusMetricLine("deeprun_server_lifecycle_state", serverLifecycleState === state ? 1 : 0, { state }));
    }
    ...
    for (const bucket of metricsHistogramBuckets) {
        lines.push(formatPrometheusMetricLine("deeprun_http_request_duration_ms_bucket", bucket.count, {
    ...
    lines.push(formatPrometheusMetricLine("deeprun_readiness_last_check_success", metricsReadinessHasSample ? (metricsReadinessLastOk ? 1 : 0) : -1));
    ...
    lines.push(formatPrometheusMetricLine("deeprun_readiness_last_check_duration_ms", metricsReadinessHasSample ? metricsReadinessLastDurationMs.toFixed(3) : 0));
    lines.push("");
    return lines.join("\n");
}
app.use((req, res, next) => {
...
function isOperationalProbePath(pathname) {
...
function suggestProjectNameFromGoal(goal) {
    const slug = goal
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "")
...
function extractValidationViolations(checks) {
    const violations = [];
    for (const check of checks) {
        ...
        const detailViolations = check.details.violations;
...
function deriveManualFileWriteResultFromRun(input) {
    ...
    const sortedSteps = [...input.detail.steps].sort((left, right) => left.stepIndex - right.stepIndex || left.attempt - right.attempt || left.createdAt.localeCompare(right.createdAt));
...
function deriveBuilderResultFromRun(input) {
    ...
    if (run.status !== "complete" || mutationStep.status !== "completed") {
        const reason = mutationStep.errorMessage || run.errorMessage || `Builder ${input.fallbackMode} run did not complete successfully.`;
    ...
    const commands = Array.isArray(output.runCommands)
        ? output.runCommands.filter((entry) => typeof entry === "string")
        : [];
    const filesChanged = Array.isArray(output.stagedDiffs)
        ? output.stagedDiffs
            .map((entry) => {
            ...
            .filter((value) => Boolean(value))
        : [];
    const resolvedModel = typeof output.model === "string" && output.model.trim()
        ? output.model.trim()
        : run.model || "unknown";
    return {
        summary,
...
app.get("/api/ready", async (_req, res) => {
    ...
    try {
        await withTimeout(store.ping(), readinessDbTimeoutMs, `Database readiness check timed out after ${readinessDbTimeoutMs}ms.`);
...
app.post("/api/projects/:projectId/deployments", authRequired, async (req, res, next) => {
    try {
        ...
        const runValidationResult = run.validationResult && typeof run.validationResult === "object" && !Array.isArray(run.validationResult)
            ? run.validationResult
            : null;
        const v1ReadyRaw = runValidationResult &&
            typeof runValidationResult.v1Ready === "object" &&
            runValidationResult.v1Ready !== null &&
            !Array.isArray(runValidationResult.v1Ready)
            ? runValidationResult.v1Ready
            : null;
