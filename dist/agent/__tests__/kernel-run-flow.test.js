...
import { buildExecutionContractMaterial, EXECUTION_CONFIG_SCHEMA_VERSION, hashExecutionContractMaterial } from "../execution-contract.js";
...
if (!databaseUrl) {
    throw new Error("Kernel flow tests require DATABASE_URL or TEST_DATABASE_URL. Example: postgres://postgres:postgres@localhost:5432/deeprun_test");
}
const requiredDatabaseUrl = databaseUrl;
function executionConfigProfile(profile) {
    if (profile === "ci") {
        return {
...
test("crash replay appends retry attempt for same step index", async () => {
    ...
    try {
        ...
        const replayReady = (await harness.store.updateAgentRun(started.run.id, {
            status: "failed",
            currentStepIndex: 1,
...
test("dirty worktree recovery resets to last valid commit before replay", async () => {
    ...
    try {
        ...
        assert.ok(started.run.worktreePath);
        const worktreePath = started.run.worktreePath;
        const lastValidCommit = started.run.lastValidCommitHash;
...
test("correction policy engine blocks malformed correction metadata in enforce mode", async () => {
    ...
    try {
        ...
        assert.equal(correctionStep?.outputPayload.correctionPolicy?.ok, false);
        assert.match(correctionStep?.outputPayload.correctionPolicy?.summary || "", /correction_attempt_suffix_match/);
...
test("runtime correction loop stops at configured goal-phase limit", async () => {
    ...
    try {
        ...
        assert.equal(started.run.errorDetails?.category, "runtime_correction_limit");
        ...
        const failureDetails = failedVerifyStep?.outputPayload.failureDetails;
...
test("run-scoped execution config overrides env runtime correction limits", async () => {
    ...
    try {
        ...
        assert.deepEqual(started.run.metadata.executionConfig, {
...
test("queueResumeRun preserves persisted execution config when unchanged", async () => {
    ...
    try {
        ...
        assert.deepEqual(resumed.run.metadata.executionConfig, persistedExecutionConfig);
    }
    finally {
        if (previousLightValidation === undefined) {
            delete process.env.AGENT_LIGHT_VALIDATION_MODE;
...
test("queueResumeRun attaches execution config to legacy runs on first continue", async () => {
    ...
    try {
        ...
        const executionConfig = resumed.run.metadata.executionConfig;
...
test("queueResumeRun rejects execution config mismatch without override or fork", async () => {
    ...
    try {
        ...
        await assert.rejects(() => kernel.queueResumeRun({
            project: harness.project,
            runId: run.id,
...
test("queueResumeRun forks a new run when execution config changes explicitly", async () => {
    ...
    try {
        ...
        const sourceExecutionConfig = (sourceRun?.metadata).executionConfig;
        const forkExecutionConfig = forked.run.metadata.executionConfig;
...
test("executeRunJob fails closed when stored execution contract hash mismatches persisted config", async () => {
    ...
    try {
        ...
        await assert.rejects(() => kernel.executeRunJob({
...
test("executeRunJob refuses unsupported future execution contracts deterministically", async () => {
    ...
    try {
        ...
        const unsupportedMaterial = {
            ...buildExecutionContractMaterial(persistedExecutionConfig),
...
test("worker lease reclaim completes terminal jobs without duplicating side effects", async () => {
    ...
    try {
        ...
        const learningCountAfterFirst = Number((await harness.store.query(`SELECT COUNT(*)::text AS count FROM learning_events WHERE run_id = $1`, [queued.run.id]))[0]?.count || 0);
        ...
        await harness.store.query(`UPDATE run_jobs
       SET lease_expires_at = NOW() - interval '1 second'
       WHERE id = $1`, [claimedByA.id]);
        await harness.store.upsertWorkerNodeHeartbeat({
            nodeId: "worker-b",
            role: "compute",
            status: "online",
        ...
        const learningCountAfterSecond = Number((await harness.store.query(`SELECT COUNT(*)::text AS count FROM learning_events WHERE run_id = $1`, [queued.run.id]))[0]?.count || 0);
